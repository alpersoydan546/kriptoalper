...
SYMBOLS = [
    ...
    "DOGEUSDT","1000SHIBUSDT","1000PEPEUSDT","1000FLOKIUSDT","1000BONKUSDT","WIFUSDT","MEMEUSDT","ORDIUSDT","1000SATSUSDT",
...
def calc_signal(symbol):
    ...
    try:
        ...
        # --- YENİ STRATEJİ: MOMENTUM (RSI + STOCH) ---
        # Tasarım aynı kalsın diye logic'i buraya gizledik.
        ...
        # LONG: RSI < 35 VE Stoch Dipte (< 20)
        if rsi < 35 and k < 20:
            direction = "LONG"
            score = 60 + (35 - rsi) 

        # SHORT: RSI > 65 VE Stoch Tepede (> 80)
        elif rsi > 65 and k > 80:
            direction = "SHORT"
            score = 60 + (rsi - 65) 
        ...
        if direction:
            ...
            # Tasarımın aynen korundu dostum
...
def run(token, chat):
    ...
    # --- BAŞLANGIÇ MESAJI (ASLAN) ---
    tg_send("🦁 <b>KriptoAlper Hunter v8.1 Av Başladı</b>\n(Mod: Hiper-Aktif / Hızlandırılmış)")
    
    # Test Mesajı
    test_df = fetch_data("BTCUSDT", "15m")
    if test_df is not None:
        tg_send(f"✅ <b>BAĞLANTI BAŞARILI!</b>\nAslan sahaya indi. Tarama başladı...")
    else:
    ...
    while True:
        try:
            if error_count > 10:
                tg_send("⚠️ <b>DİKKAT:</b> Veri akışı koptu! Sunucuyu kontrol et.")
                error_count = 0 

            check_results() 
            send_daily_summary() 
            
            # --- 4 SAATLİK NÖBET MESAJI (SEÇENEK C: İZ SÜRÜCÜ) ---
            if datetime.now() - last_health_check > timedelta(hours=2): # Hızlandığı için 2 saate çektim
