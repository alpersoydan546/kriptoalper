...
# --- [ PIRANHA v19.8 - GROK EDITION ] ---
# Grok Optimizasyonu: Confidence 90 + sadece 7 ana coin (spam ve fake stop azalacak)
# --- AYARLAR ---
TIMEFRAME = '5m'
LOOKBACK = 50
...
WICK_RATIO = 2.5 
RISK_REWARD = 1.5
CONFIDENCE_THRESHOLD = 90   # <--- DEĞİŞTİ (fake stop azalacak)
# --- LİMİTLER ---
SCAN_INTERVAL = 30
...
def run(token=None, chat_id=None):
    ...
    logger.info("☁️ PIRANHA v19.8 ONLINE (GROK EDITION)")
    send_telegram("☁️ Piranha v19.8 Aktif (Grok Zırhı Devrede)")
   
    last_report_day = datetime.now().day
    target_list = ["BTC/USDT", "ETH/USDT", "SOL/USDT", "BNB/USDT", "XRP/USDT", "DOGE/USDT", "ADA/USDT"]  # <--- SABİT 7 COIN
    while True:
        try:
            check_active_trades()
